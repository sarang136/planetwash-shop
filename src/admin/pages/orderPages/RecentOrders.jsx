...
const RecentOrders = () => {
  ...
  const pickupOrders = allOrders.filter(order => order.orderStatus === "pending");
  const navigate = useNavigate();
console.log("allorders",pickupOrders);
  ...
  return (
    <div className="overflow-x-auto max-h-[75vh] overflow-y-auto px-4 bg-white rounded-2xl shadow">
      {pickupOrders.length === 0 ? (
        ...
        <table className=" w-full  text-xs md:text-sm bg-white rounded-xl">
                ...
                <tr key={_id || index} className="text-gray-400">
                  ...
                  <td className="p-2">
                    <button
                      ...
                      onClick={() => navigate('/map', { state: { orderId: _id , locs: addressId} })}
                    >
                      Schedule Pickup
                    </button>
