...
const PickupSchedule = () => {
  ...
  const pickupOrders = allOrders.filter(
    (order) => order.orderStatus?.toLowerCase() === "rejected"
  );
  ...
  return (
    <div className="overflow-x-auto max-h-[75vh] overflow-y-auto px-4 bg-white rounded-2xl shadow">
      {pickupOrders.length === 0 ? (
        <p className="text-center text-gray-500 py-6">No Rejected Orders found</p>
      ) : (
            ...
            {pickupOrders.map((order, index) => {
              ...
              return (
                <tr key={_id || index} className=" text-gray-400">
                  <td className="p-3 cursor-pointer" onClick={() => navigate(`/userData/${_id}`)}>
                    {addressId?.name || "N/A"}
                  </td>
                  ...
                  <td className="p-3">{addressId?.location}, {addressId?.pincode}</td>
